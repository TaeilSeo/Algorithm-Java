	...
	private static int bfs(Data pos) {
		...
		while( !q.isEmpty() ) {
			...
			for(int d=0; d<4; d++) { // 4방 탐색
				...
				if(nr >=0 && nr < R && nc >= 0 && nc < C) { // 2차원 배열의 범위를 벗어나지 않을 때,
					if(!ch[nr][nc] && grid[nr][nc] == 'L' ) { // 진행할 수 있는 좌표가 있다면
						ch[nr][nc] = true; // 방문체크
						q.offer(new Data(nr, nc, cur.dist + 1)); // 큐에 삽입
					} else { // 더 이상 진행할 수 있는 좌표가 없다면
						max = Integer.max(max, cur.dist); // 최댓값을 구하기 위하여 pos 좌표로부터의 거리를 비교
					}
				}
			}
		}
		
		return max; // pos 좌표 기준 bfs 탐색에서의 최대 거리 값 반환
	}
